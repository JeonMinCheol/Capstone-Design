...
public class CouchSort extends Sort implements CouchRel {
  protected CouchSort(RelOptCluster cluster, RelTraitSet traitSet,
  ...
  @Override public Sort copy(RelTraitSet traitSet, RelNode input,
    ...
    return new CouchSort(getCluster(), traitSet, input, collation, offset, fetch);
  }
  ...
  public void implement(Implementor implementor) {
    ...
    if (!collation.getFieldCollations().isEmpty()) {
      ...
      for (RelFieldCollation fieldCollation : collation.getFieldCollations()) {
        ...
        sortList.add("{" + CouchRules.quote(rawName) + ": " + ((direction(fieldCollation) == 1) ? "\"asc\"" : "\"desc\"") + "}");
      ...
      if (offset != null) {
        implementor.setSkip(Integer.parseInt(String.valueOf(offset)));
      }

      implementor.setSortString("\"sort\" : " + Util.toString(sortList, "[", ", ", "]"));
    }
  }
